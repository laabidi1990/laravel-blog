<div class="col-md-4 col-xl-3">
    ...
    <form class="input-group" action="" method="GET" name="myform">
        <input type="text" class="form-control" name="search" placeholder="Search" value="{{ request()->query('search') }}">
        <div class="input-group-addon">
          <span class="input-group-text"> 
            <i class="ti-search" style="cursor:pointer" onclick="myform.submit()"></i>
          </span>
        </div>
      </form>
      ...
      <div class="row link-color-default fs-14 lh-24">
          @foreach ($categories as $category)
              <div class="col-6"><a href="{{ route('categories-posts', $category->id)}}">{{ $category->name}}</a></div>
          @endforeach
      </div>
      ...
      <div class="gap-multiline-items-1">
          @foreach ($tags as $tag)
          <a class="badge badge-secondary" href="{{ route('tags-posts', $tag->id) }}">{{ $tag->name}}</a>
          @endforeach
      </div>
